...
-- (+ мод: сообщения только 8 пользователя: WHERE u.id = 8;)
...
-- Выбрать фотографии (filename) у юзера c почтой arlo50@example.org.
-- ID типа медиа мы не знаем 
SELECT filename
FROM media
...
-- Список медиафайлов пользователей с количеством лайков (используя JOIN)
SELECT 
	m.id,  
