...
SELECT c.shops_id, s.id, c.name, s.shopname
FROM cats c 
JOIN shops s 
ON c.shops_id = s.id;
...
WHERE id IN (SELECT id FROM cats WHERE name = 'Murzik');
...
SELECT o.ord_id, a.an_name, a.an_cost, a.an_price, o.ord_datetime
FROM Analysis a
JOIN Orders o
ON a.an_id = o.ord_an
